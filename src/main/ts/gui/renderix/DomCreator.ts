...
export default class DomCreator {
  ...
  private static getMarkupFromComponent(data: ElementData, component: RenderixComponent, context: RenderixContext) {
    switch ((data as any).length) {
      case 1: //tagname only
      ...
      case 2: {  //tagname and options
        ...
        const result = component.render(attributes, null);
        DomCreator.applyEffectsToComponent(component, attributes, context);
        return result;
      }

      case 3: {
        const attributes = data[1] || {};
        const result = component.render(attributes, data[2]);
        DomCreator.applyEffectsToComponent(component, attributes, context);
        return result;
      }
    }
